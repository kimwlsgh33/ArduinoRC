...
String folderPath = "/home/바탕화면/3D_Printed_Arduino_RC_CAR-4772279/images/"; // 이미지들이 저장된 폴더 경로를 정의
...
void template_matching()
{
	...
	img = img + Scalar(50, 50, 50); // 메인 이미지를 밝게 하기 위해 각 픽셀 값에 스칼라 값을 더함

	Mat noise(img.size(), CV_32SC3);   // 메인 이미지와 같은 크기의 노이즈 행렬을 생성
	randn(noise, 0, 10);      // 평균 0, 표준편차 10인 정규분포를 따르는 난수(정의된 범위 내에서 무작위로 추출된 수)를 노이즈 행렬에 채움
	add(img, noise, img, Mat(), CV_8UC3);   // 노이즈를 메인 이미지에 추가

	Mat res, res_norm;   // 템플릿 매칭 결과와 정규화된 결과를 위한 행렬
	matchTemplate(img, templ, res, TM_CCOEFF_NORMED);      // TM_CCOEFF_NORMED 방법을 사용하여 템플릿 매칭 수행
	normalize(res, res_norm, 0, 255, NORM_MINMAX, CV_8U);   // 결과를 [0, 255] 범위로 정규화
